...
describe('launchers/process.js', () => {
  ...
  beforeEach(() => {
    ...
    logErrorSpy = sinon.stub(logger.create('launcher'), 'error')
    logDebugSpy = sinon.stub(logger.create('launcher'), 'debug')
  ...
  describe.only('with RetryLauncher', () => {
    function assertSpawnError ({ errorCode, emitExit, expectedError }, done) {
      ...
      _.defer(() => {
        ...
        expect(logDebugSpy).to.have.been.callCount(3)
        expect(logDebugSpy.getCall(0)).to.have.been.calledWithExactly(`${BROWSER_PATH} http://host:9876/?id=fake-id`)
        expect(logDebugSpy.getCall(1)).to.have.been.calledWithExactly('Process fake-name exited with code -1')
        expect(logDebugSpy.getCall(2)).to.have.been.calledWithExactly('fake-name failed (cannot start). Not restarting.')
